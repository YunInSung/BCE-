...
def P_matrix_tf(X, Y, W1, b1, W2, b2, learn):
    ...
    def process_hidden_row(row_idx):
        ...
        grad_W1_r_ = grad_W1_func_r_tf(theta, X, Xi)    # (D+1, D)
        H_r = H_matirx1_r_tf(theta, X, Xi, grad_W1_r_)    # (D+1, D+1)
        L_r = L_matrix1_r_tf(X, P1, theta, dW1, db1, row_idx, Xi, grad_W1_r_, learn)  # (1, D+1)
    ...
    matrix1 = tf.vectorized_map(lambda idx: process_hidden_row(idx), hidden_rows)
    ...
    def process_output_row(row_idx):
        grad_W2_r_ = grad_W2_func_r_tf(A1, row_idx, d2Z2, A1i)  # (m+1, m)
        H_r = H_matirx2_r_tf(A1, row_idx, d2Z2, A1i, grad_W2_r_)  # (m+1, m+1)
        L_r = L_matrix2_r_tf(A1, P2, row_idx, dW2, db2, d2Z2, A1i, grad_W2_r_, learn)  # (1, m+1)
        H_r_reg = H_r + epsilon * tf.eye(tf.shape(H_r)[0], dtype=tf.float32)
        P_r = tf.transpose(tf.linalg.solve(tf.transpose(H_r_reg), tf.transpose(L_r)))
        return P_r  # 예상 shape: (1, m+1)

    output_rows = tf.range(p)  # p = tf.shape(Y)[0]
